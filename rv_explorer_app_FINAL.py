...
# Set page config
st.set_page_config(
    page_title="Realized Volatility Explorer",
    page_icon="📊",
    layout="wide",
...
@st.cache_data
def process_uploaded_data(file_bytes, time_col, close_col, max_jump=0.1):
    """Parse and process user-uploaded CSV/Excel into RV series."""
    try:
        # Detect file type from bytes magic or try both
        try:
        ...
        if time_col not in df.columns:
            return None, f"Column '{time_col}' not found. Available columns: {list(df.columns)}"
        if close_col not in df.columns:
            return None, f"Column '{close_col}' not found. Available columns: {list(df.columns)}"
        ...
        price_1m = pd.to_numeric(df[close_col], errors="coerce").dropna()

        if len(price_1m) < 10:
            return None, "Not enough valid rows after parsing. Check your timestamp and close column selections."

        # Log returns and jump filter
        log_ret_1m = np.log(price_1m).diff()
        impossible_jumps = log_ret_1m.abs() > max_jump
        ...
        # Yang-Zhang
        ohlc_daily = price_1m.resample("1D").agg(open="first", high="max", low="min", close="last").dropna()
        ...
        YZ_vol_daily = np.sqrt(r_O**2 + 0.34 * r_C**2 + 0.66 * RS).dropna()
        ...
        info = {
            'rows': len(price_1m),
            'days': len(RV_1m_vol),
            'jumps_removed': int(impossible_jumps.sum()),
            'start': price_1m.index.min(),
            'end': price_1m.index.max(),
        }
        return data, info
...
def generate_demo_data():
    """Generate demo data when no file is uploaded."""
    dates = pd.date_range(start='2020-01-01', end='2024-12-31', freq='D', tz='UTC')
    np.random.seed(42)

    RV_1m_var = pd.Series(np.random.gamma(2, 0.0004, len(dates)) + 0.0001, index=dates)
    RV_5m_var = pd.Series(np.random.gamma(2, 0.00025, len(dates)) + 0.00008, index=dates)
...
def main():
    # Header
    st.markdown('<p class="main-header">📊 Realized Volatility Explorer</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Interactive Analysis of Volatility Dynamics — upload your own data or explore the demo</p>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "Upload your price data (CSV or Excel)",
            type=["csv", "xlsx", "xls"],
            help="File must contain a timestamp column and a close-price column. "
                 "Timestamps can be Unix seconds (integers) or any date string."
        )
        ...
        if uploaded_file is not None:
            # Column name pickers – shown after upload so user can inspect headers
            with st.expander("⚙️ Column mapping", expanded=True):
                # Peek at columns without processing
                try:
                    _peek = pd.read_csv(uploaded_file, nrows=0)
                    uploaded_file.seek(0)
                    col_options = list(_peek.columns)
                except Exception:
                    uploaded_file.seek(0)
                    try:
                        _peek = pd.read_excel(uploaded_file, nrows=0)
                        uploaded_file.seek(0)
                        col_options = list(_peek.columns)
                    except Exception:
                        col_options = []

                default_time = next((c for c in col_options if "time" in c.lower() or "date" in c.lower()), col_options[0] if col_options else "Timestamp")
                default_close = next((c for c in col_options if "close" in c.lower()), col_options[-1] if col_options else "Close")

                time_col = st.selectbox("Timestamp column", col_options if col_options else ["Timestamp"], index=col_options.index(default_time) if default_time in col_options else 0)
                close_col = st.selectbox("Close price column", col_options if col_options else ["Close"], index=col_options.index(default_close) if default_close in col_options else 0)
                max_jump = st.slider("Max 1-min log-return (jump filter)", 0.01, 0.30, 0.10, 0.01,
                                     help="Returns larger than this are treated as data errors and removed.")

            with st.spinner("Processing uploaded data…"):
                result, meta = process_uploaded_data(uploaded_file, time_col, close_col, max_jump)

            if result is None:
                st.error(f"❌ Could not process file: {meta}")
                ...
                data = result
                st.success(
                    f"✅ Loaded **{meta['rows']:,}** rows → **{meta['days']}** trading days\n\n"
                    f"📅 {meta['start'].date()} → {meta['end'].date()}\n\n"
                    f"🔧 {meta['jumps_removed']} impossible jumps removed"
                )
        else:
            st.info("No file uploaded — showing **demo data**.\n\nUpload a 1-minute OHLCV CSV above to analyse your own asset.")
            data = generate_demo_data()
            using_demo = True

        if using_demo:
            st.caption("ℹ️ Demo data is randomly generated and for illustration only.")
    ...
    ret_d = returns_daily[ret_mask].dropna()

    if rv_type == "1-minute RV":
        rv_var_full = data["RV_1m_var"]
    elif rv_type == "5-minute RV":
        rv_var_full = data["RV_5m_var"]
    else:
        rv_var_full = data["RV_5m_var"]

    rv_var_full = rv_var_full[(rv_var_full.index >= start_timestamp) & (rv_var_full.index <= end_timestamp)].dropna()

    # Prepare HAR dataset exactly like report
    har_df = pd.DataFrame({"RV": rv_var_full.copy()})
    har_df["RV_d1"] = har_df["RV"].shift(1)
    har_df["RV_w5"] = har_df["RV"].rolling(5).mean().shift(1)
    har_df["RV_m22"] = har_df["RV"].rolling(22).mean().shift(1)
    har_df["RV_next"] = har_df["RV"].shift(-1)
    har_df = har_df.dropna()

    X_cols = ["RV_d1", "RV_w5", "RV_m22"]

    # Train/test split by date boundary, same as report
    split_idx = int(len(har_df) * 0.8)

    if split_idx < 30 or len(har_df) - split_idx < 10:
        st.warning("Not enough data to run the report-identical HAR VaR backtest.")
    else:
        split_date = har_df.index[split_idx]

        train_df = har_df.loc[har_df.index <= split_date].copy()
        test_df = har_df.loc[har_df.index > split_date].copy()

        rv_hat = []
        forecast_dates = []

        for dt in test_df.index:
            fit_df = pd.concat([train_df, test_df.loc[test_df.index < dt]])
            fit_df = fit_df.iloc[-1825:]  # last 5 years, identical to report

            y = fit_df["RV_next"]
            X = sm.add_constant(fit_df[X_cols], has_constant="add")
            model = sm.OLS(y, X).fit()

            X_new = test_df.loc[[dt], X_cols]
            X_new = sm.add_constant(X_new, has_constant="add")
            X_new = X_new[model.params.index]

            rv_pred = model.predict(X_new).iloc[0]
            rv_hat.append(max(rv_pred, 0.0))
            forecast_dates.append(dt)

        rv_hat = pd.Series(rv_hat, index=forecast_dates, name="RV_hat")
        sigma_hat = np.sqrt(rv_hat)

        # Align returns with forecast dates, identical to report
        common_index = ret_d.index.intersection(sigma_hat.index)
        ret_test = ret_d.loc[common_index]
        sigma_hat = sigma_hat.loc[common_index]
        ...
        if var_dist == "Normal":
            ...
            q = t.ppf(var_alpha, df=var_df_t)

        var_series = mu_hat + sigma_hat * q

        if var_position == "Short":
            var_series = -var_series
            exceptions = (ret_test > var_series)
        else:
            exceptions = (ret_test < var_series)

        col1, col2, col3 = st.columns(3)
        col1.metric("Test Observations", f"{len(exceptions):,}")
        col2.metric("Exceptions", f"{int(exceptions.sum()):,}")
        col3.metric("Expected", f"{var_alpha * len(exceptions):.1f}")

        actual_rate = exceptions.mean()
        expected_rate = var_alpha

        if actual_rate > expected_rate * 1.5:
            st.markdown(
                f'<div class="warning-box">⚠️ Exception rate {actual_rate*100:.2f}% exceeds expected {expected_rate*100:.2f}%.</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f'<div class="insight-box">✅ Exception rate {actual_rate*100:.2f}% is close to expected {expected_rate*100:.2f}%.</div>',
                unsafe_allow_html=True
            )
        ...
        st.dataframe(
            pd.DataFrame({
                "Return": ret_test,
                "VaR": var_series,
                "Exception": exceptions.astype(int)
            }).tail(20),
